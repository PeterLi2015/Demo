...
namespace XDropsWater.CrossCutting.BarCode
{
    public class Code128Rendering
    {
        ...
        public static Image MakeBarcodeImage(string InputData, int BarWeight, bool AddQuietZone, bool isShowCode)
        {
            ...
            //height = Convert.ToInt32( System.Math.Ceiling( Convert.ToSingle(width) * 1F) );
            height = 40;
            rectHeight += height + (isShowCode ? 20 : 0);
            if (AddQuietZone)
            {
            ...
            Image myimg = new System.Drawing.Bitmap(width, rectHeight);
            using (Graphics gr = Graphics.FromImage(myimg))
            {
                ...
                if (isShowCode)
                {
                    gr.FillRectangle(System.Drawing.Brushes.White, 0, height, width, rectHeight - height);
                    Font font = new Font("Verdana", 13f, FontStyle.Bold);
                    SizeF size = gr.MeasureString(InputData, font);
                    gr.DrawString(InputData, font, Brushes.Black, new Point((Int32)((width - size.Width) / 2), height));
                }
            }
