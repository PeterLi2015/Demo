...
var rows = new Vue({
    ...
    methods: {
        viewCode: function (item, index) {
            showCodes(item, index);
        },
        addCode: function (item, index) {
            showAddCode(item, index);
        },
...
function sendReset(order, index) {
    ...
    var title = '【' + order.Member.MemberName + '】订货【' + order.Quantity + '】套 -- 发货';
    var operateType = '发货';
...
function send(model) {
    ...
    var title = '发货后订单数据不能更改，您确定要为【' + model.MemberName + '】的订单【' + model.Quantity + '】套发货吗？'
    showConfirm(title, function () {
        sendOut(model)
    });
